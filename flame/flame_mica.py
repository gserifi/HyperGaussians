...
class FLAME_mica(nn.Module):
    ...
    def __init__(self, config):
        ...
        shapedirs = torch.cat(
            [shapedirs[:, :, : config.num_shape_params], shapedirs[:, :, 300 : 300 + config.num_exp_params]], 2
        )
        ...
        self.register_buffer(
            "l_eyelid",
            torch.from_numpy(np.load(f"{os.path.abspath(os.path.dirname(__file__))}/blendshapes/l_eyelid.npy")).to(
                self.dtype
            )[None],
        )
        self.register_buffer(
            "r_eyelid",
            torch.from_numpy(np.load(f"{os.path.abspath(os.path.dirname(__file__))}/blendshapes/r_eyelid.npy")).to(
                self.dtype
            )[None],
        )
        ...
        mediapipe_lmk_embedding = np.load(
        ...
        self.register_buffer(
            "mp_lmk_faces_idx", torch.from_numpy(mediapipe_lmk_embedding["lmk_face_idx"].astype(int)).to(torch.int64)
        )
        self.register_buffer(
            "mp_lmk_bary_coords", torch.from_numpy(mediapipe_lmk_embedding["lmk_b_coords"]).to(self.dtype).float()
        )

        self.register_buffer(
            "lmk_faces_idx", torch.from_numpy(lmk_embeddings["static_lmk_faces_idx"].astype(int)).to(torch.int64)
        )
        self.register_buffer(
            "lmk_bary_coords", torch.from_numpy(lmk_embeddings["static_lmk_bary_coords"]).to(self.dtype).float()
        )
        self.register_buffer(
            "dynamic_lmk_faces_idx",
            torch.from_numpy(np.array(lmk_embeddings["dynamic_lmk_faces_idx"]).astype(int)).to(torch.int64),
        )
        self.register_buffer(
            "dynamic_lmk_bary_coords",
            torch.from_numpy(np.array(lmk_embeddings["dynamic_lmk_bary_coords"])).to(self.dtype).float(),
        )
    ...
    def _find_dynamic_lmk_idx_and_bcoords(
        self, vertices, pose, dynamic_lmk_faces_idx, dynamic_lmk_b_coords, neck_kin_chain, cameras, dtype=torch.float32
    ):
        """
        ...
        rel_rot_mat = (
            cameras @ rel_rot_mat
        )  # Cameras flips z and x, plus multiview needs different lmk sliding per view
