...
from src.hypergaussians import HyperGaussians
from utils.general_utils import Embedder, Pytorch3dRasterizer, a_in_b_torch, face_vertices_gen, load_binary_pickle


class HyperGaussians_Deform_Model(nn.Module):
    def __init__(self, lpt, device):
        super().__init__()
        self.device = device
        self.hidden_dim = lpt.hidden_dim
        self.hidden_layers = lpt.hidden_layers
        self.latent_dim = lpt.latent_dim
        self.num_gaussians = 14876  # Won't change
        self.conditional_appearance = lpt.conditional_appearance
        self.camera_conditioning = lpt.camera_conditioning
        ...
        self.uvcoords = torch.cat([uv_coords, uv_coords[:, :, 0:1] * 0.0 + 1.0], -1).to(self.device)
    ...
    def init_networks(self):
        ...
        self.deformNet = MLP(
            ...
            output_dim=self.latent_dim if not self.conditional_appearance else 2 * self.latent_dim,
            hidden_dim=self.hidden_dim,
            hidden_layers=self.hidden_layers,
        )

        self.hgs_xyz = HyperGaussians(self.num_gaussians, self.latent_dim, 3)
        self.hgs_rot = HyperGaussians(self.num_gaussians, self.latent_dim, 4)
        self.hgs_scl = HyperGaussians(self.num_gaussians, self.latent_dim, 3)

        if self.conditional_appearance:
            camera_latent = 3 if self.camera_conditioning else 0
            self.hgs_clr = HyperGaussians(self.num_gaussians, self.latent_dim + camera_latent, 3)
            self.hgs_opc = HyperGaussians(self.num_gaussians, self.latent_dim + camera_latent, 1)

    def example_init(self, codedict):
        # speed up
        shape_code = codedict["shape"].detach()
        batch_size = shape_code.shape[0]
        geometry_shape = self.flame_model.forward_geo(shape_code, expression_params=self.default_expr_code)
        ...
        rast_out, pix_to_face, bary_coords = self.uv_rasterizer(
            self.uvcoords.expand(batch_size, -1, -1), self.uvfaces.expand(batch_size, -1, -1), face_vertices_shape
        )
    ...
    def decode(self, codedict):
        ...
        latent = self.deformNet(uv_vertices_shape_embeded_condition)

        if self.conditional_appearance:
            geometry_latent = latent[:, :, : self.latent_dim]
            appearance_latent = latent[:, :, self.latent_dim :]
        else:
            geometry_latent = latent

        deforms_xyz, uncertainty_xyz = self.hgs_xyz(geometry_latent)
        deforms_rot, uncertainty_rot = self.hgs_rot(geometry_latent)
        deforms_scl, uncertainty_scl = self.hgs_scl(geometry_latent)

        deforms = torch.cat((deforms_xyz, deforms_rot, deforms_scl), dim=-1)
        ...
        if self.conditional_appearance:
            if self.camera_conditioning:
                camera_center = codedict["camera_center"].detach()
                dir_pp = verts_final - camera_center.repeat(latent.shape[0], 1).unsqueeze(0)
                dir_pp_normalized = dir_pp / dir_pp.norm(dim=1, keepdim=True)
                appearance_latent = torch.cat((appearance_latent, dir_pp_normalized), dim=-1)

            color, uncertainty_clr = self.hgs_clr(appearance_latent)
            opacity, uncertainty_opc = self.hgs_opc(appearance_latent)
        ...
        uncertainty = uncertainty_xyz + uncertainty_rot + uncertainty_scl
        uncertainty = uncertainty[:, self.uv_head_idx, None]

        if self.conditional_appearance:
            color = color[:, self.uv_head_idx, :]
            opacity = opacity[:, self.uv_head_idx, :]

            return verts_final, rot_delta, scale_coef, uncertainty, color, opacity

        return verts_final, rot_delta, scale_coef, uncertainty

    def capture(self):
        return (
            self.deformNet.state_dict(),
            self.hgs_xyz.state_dict(),
            self.hgs_rot.state_dict(),
            self.hgs_scl.state_dict(),
            *([self.hgs_clr.state_dict(), self.hgs_opc.state_dict()] if self.conditional_appearance else []),
    ...
    def restore(self, model_args):
        ...
        self.hgs_xyz.load_state_dict(hgs_xyz_dict)
        self.hgs_rot.load_state_dict(hgs_rot_dict)
        self.hgs_scl.load_state_dict(hgs_scl_dict)

        if self.conditional_appearance:
            self.hgs_clr.load_state_dict(hgs_clr_dict)
            self.hgs_opc.load_state_dict(hgs_opc_dict)
    ...
    def training_setup(self):
        params_group = [
            {"params": self.deformNet.parameters(), "lr": 1e-4, "name": "deformNet"},
            {"params": self.hgs_xyz.parameters(), "lr": 1e-4, "name": "hgs_xyz"},
            {"params": self.hgs_rot.parameters(), "lr": 1e-4, "name": "hgs_rot"},
            {"params": self.hgs_scl.parameters(), "lr": 1e-4, "name": "hgs_scl"},
            *(
                [
                    {"params": self.hgs_clr.parameters(), "lr": 1e-4, "name": "hgs_clr"},
                    {"params": self.hgs_opc.parameters(), "lr": 1e-4, "name": "hgs_opc"},
                ]
                if self.conditional_appearance
                else []
            ),
...
class MLP2(nn.Module):
    def __init__(self, input_dim, condition_dim, output_dim1, output_dim2, hidden_dim=256, hidden_layers=8):
        ...
        self.hidden_layers = hidden_layers
        mid_layers = math.ceil(hidden_layers / 2.0)
        self.input_dim = input_dim
        ...
        self.fcs1 = nn.ModuleList(
            ...
            + [
                (
                    ...
                    if i != (mid_layers - 1)
                    else nn.Linear(hidden_dim, hidden_dim + output_dim1)
                )
        ...
        self.fcs2 = nn.ModuleList(
            [
                nn.Linear(hidden_dim, hidden_dim) if i != 0 else nn.Linear(hidden_dim + condition_dim, hidden_dim)
                for i in range(mid_layers, hidden_layers - 1)
            ]
        )
...
class SIRENMLP(nn.Module):
    def __init__(self, input_dim=3, output_dim=3, hidden_dim=256, hidden_layers=8, condition_dim=100, device=None):
        super().__init__()
        ...
        self.mapping_network = MappingNetwork(condition_dim, 256, len(self.network) * self.hidden_dim * 2)

        self.network.apply(frequency_init(25))
        # self.final_layer.apply(frequency_init(25))
        self.final_layer.weight.data.normal_(0.0, 0.0)
    ...
    def init(m):
        with torch.no_grad():
            if isinstance(m, nn.Linear):
                ...
                num_input = torch.prod(torch.tensor(m.weight.shape[1:], device=m.weight.device)).cpu().item()
                m.weight.uniform_(-np.sqrt(6 / num_input) / freq, np.sqrt(6 / num_input) / freq)
